
Science_Vessel = ("Science Vessel", 0, 16, (3,100,200), 0, True, None, True, None, True, True, "images/science_vessel.png")

Marine = ("Marine", 3, 5, (1,50,0), 0, None, None, None, 1, None, None, "images/marine.png")
Firebat = ("Firebat", 3, 5, (1,50,25), 1, None, None, None, None, None, None, "images/firebat.png")
Siege_Tank = ("Siege Tank", 6, 12, (3,150,100), 0, None, None, None, None, None, None, "images/tank.png")
Wright = ("Wright", 4, 10, (2,150,100), 0, True, None, True, 0.5, None, None, "images/wright.png") 
Battlecruiser = ("Battlecruiser", 8, 36, (8, 400, 300), 0, None, None, True, 1, None, None, "images/Battlecruiser.png")
# IDEAS:
#Goliath ("Goliath", 4, 10, (2,100,50), 0, None, None, None, 1, None, None,  ) # upgrade -double damage to flyers?
#Ghost = ("Ghost", 3, 3, (1,25,75), 1, None, None, None, 1, None, None) #lockdown, snipe, cloak, nuclear strike


Overlord = ("Overlord", 0, 13, (0,100,0), 0, True, None, True, None, True, True, "images/overlord.png") 

Zergling = ("Zergling", 2, 4, (1,25,0), 0, True, None, None, None, None, None, "images/zergling.png")
Hydralisk = ("Hydralisk", 4, 6, (1,75,25), 0, None, None, None, 1, None, None, "images/hydralisk.png")
Mutalisk = ("Mutalisk", 3, 10, (2,100,100), 0, True, None, True, 1, None, None, "images/mutalisk.png")
Defiler = ("Defiler", 0, 7, (2,50,150), 0, None, None, None, None, None, None, "images/defiler.png")
Ultralisk = ("Ultralisk", 8, 15, (6,300,200), 2, None, None, None, None, None, None, "images/ultralisk.png")
Lurker = ("Lurker", 5, 9, (1,100,50), 1, None, True, None, None, None, True, "images/lurker.png") 
Guardian = ("Guardian", 7, 16, (2,50,100), 0, None, None, True, 1, None, None, "images/guardian.png")


Observer = ("Observer", 0, 3, (1,25,75), 0, True, True, True, None, True, True, "images/observer.png")

Zealot = ("Zealot", 3, 8, (2,100,0),  1, None, None, None, None, None, None, "images/zealot.png")
Dragoon = ("Dragoon", 5, 12, (2,125,50), 0, None, None, None, 1, None, None, "images/dragoon.png")
Dark_Templar = ("Dark Templar", 7, 7, (3,125,150),  0, None, True, None, None, None, None, "images/dark_templar.png")
Scout = ("Scout", 6, 18, (3,250,100), 0, None, None, True, 0.5, None, None, "images/scout.png") 
Carrier = ("Carrier", 0, 32, (6,275,250), 0, None, None, True, 1, None, None, "images/carrier.png")
# IDEAS:
#Stalker = ("Stalker", 4, 10, (2,100,50), 0, True, None, None, 1, None, None, "images/dragoon.png" )  # or Active with blink?
#Immortal = ("Immortal", 6, 24, (4,275,100), 0, None, None, None, None, None, None, "images/dragoon.png" )   #shield - max dmd dlt =3 ?
#Archon = ("Archon", 8, 12, (6,300,250), 1, None, None, None, 0.5, None, None, "images/ultralisk.png")  fr 2 Dar_templars  +  HP reg to  be programmed
#Corsair  ("Corsair", 5, 15, (2,150,100), 0, True, None, True, None, None, None, "images/scout.png") 














